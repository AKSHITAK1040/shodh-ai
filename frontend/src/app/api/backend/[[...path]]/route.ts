...
async function handleProxy(request: NextRequest, context: RouteContext) {
  ...
  console.error('[Backend Proxy] All targets failed for path:', targetPath, lastError);
  return NextResponse.json(
    { message: `Failed to proxy to backend service: ${lastError?.message || 'Connection refused'}` },
    { status: 502 }
  );
}
